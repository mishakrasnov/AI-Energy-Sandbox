...
# Using the Dashboard
The Dashboard is orginized by chosing existing submission_id or typying a new one. For now, only xgboost, torch and ieeebus39 models are avaliable. On the main tab, there is a box to submit checkpoint of a model (xgboost or torch), and configuration bar for ieeebus39. Examples of models and datasets for these three types can be found in example_xgboost, example_torch and example_ieebus39 folders. To submit data, choice your dataframe saved as csv file and select target columns. To generate reports of model and data, switch to correspinding tab and push the button.

---

# Demonstration Data
The repository includes example files demonstrating the full workflow. The sample dataset corresponds to electricity consumption forecasting for a single household, based on data from:
